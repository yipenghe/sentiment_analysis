...
def read_GLASSDOOR():
    ...
    x, y = [], []
    #count = 0
    with open("data/GLASSDOOR/pos_rating_pros_and_cons", "r", encoding="utf-8") as f:
        for line in f:
            #count += 1
            #if count > 10629:
            #    break
    ...
    #count = 0
    with open("data/GLASSDOOR/neg_rating_pros_and_cons", "r", encoding="utf-8") as f:
        for line in f:
            #count += 1
            #if count > 3143:
            #    break
    ...
    print("length:{} {}".format(len(x), len(y)))
    dev_idx = int(len(x)  - len(x) * 0.15)
    test_idx =  int(len(x)  - len(x) * 0.08)
