...
def train(data, params):
    ...
    #BEGIN FOR PROS_CONS
    if params["MODEL"] == 'pros_cons':
        print("loading pros and cons glove...")
        word_vectors = {}
        original_word_vectors = source.utils.glove2dict(os.path.join(GLOVE_HOME, 'glove.6B.50d.txt'))
        word_vectors["pros"] = source.utils.glove2dict(os.path.join(GLOVE_HOME, 'glove_pros.6B.50d.txt'))
        word_vectors["cons"] = source.utils.glove2dict(os.path.join(GLOVE_HOME, 'glove_cons.6B.50d.txt'))

        wv_matrix = []
        for i in range(len(data["vocab"])):
            word = data["idx_to_word"][i]
            if word in word_vectors["pros"].keys():
                wv_matrix.append(word_vectors["pros"][word])
            else:
                if word in original_word_vectors.keys():
                    wv_matrix.append(original_word_vectors[word])
                else:
                    wv_matrix.append(np.random.uniform(-0.01, 0.01, 50).astype("float32"))

        for i in range(len(data["vocab"])):
            word = data["idx_to_word"][i]
            if word in word_vectors["cons"].keys():
                wv_matrix.append(word_vectors["cons"][word])
            else:
                if word in original_word_vectors.keys():
                    wv_matrix.append(original_word_vectors[word])
                else:
        ...
        for i in range(0, len(data["train_x"]), params["BATCH_SIZE"]):
            ...
            for sent in data["train_x"][i:i + batch_range]:
                ...
                for j in range(len(sent)):
                    if sent[j] == '.':
                        break
                pos = [data["word_to_idx"][w] for w in sent[:j]]
                cons = [data["word_to_idx"][w] + params["VOCAB_SIZE"] for w in sent[j:]]
                padding = [params["VOCAB_SIZE"] * 2 + 1] * (params["MAX_SENT_LEN"] - len(sent))
                batch_x.append(pos + cons + padding)
...
def test(data, model, params, mode="test"):
    ...
    for sent in x:
        ...
        for j in range(len(sent)):
            if sent[j] == '.':
                break
        pos = [data["word_to_idx"][w] if w in data["vocab"] else params["VOCAB_SIZE"] * 2 for w in sent[:j]]
        cons = [data["word_to_idx"][w] + params["VOCAB_SIZE"] if w in data["vocab"] else params["VOCAB_SIZE"] * 2 for w in sent[j:]]
        padding = [params["VOCAB_SIZE"] * 2 + 1] * (params["MAX_SENT_LEN"] - len(sent))
        test_x.append(pos + cons + padding)
    ...
    x = Variable(torch.LongTensor(test_x)).to(device)
    #END FOR PROS_CONS
